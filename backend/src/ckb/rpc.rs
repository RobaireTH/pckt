...
impl CkbRpc {
    ...
    pub async fn tip_header(&self) -> Result<TipHeader> {
        ...
        let hash = raw
            ...
            .ok_or_else(|| anyhow!("tip header missing hash"))?
            .to_string();
        Ok(TipHeader {
            number,
            hash,
    ...
    pub async fn send_transaction(&self, tx: Value) -> Result<String> {
        ...
        raw.as_str()
            .map(str::to_string)
            .ok_or_else(|| anyhow!("send_transaction returned non-string: {raw}"))
    }
    ...
    pub async fn get_cells(
        ...
        let limit_hex = format!("0x{limit:x}");
        let params = match cursor {
            Some(c) => json!([key, "asc", limit_hex, c]),
            None => json!([key, "asc", limit_hex]),
        };
        let raw: Value = self.call("get_cells", params).await?;
        let objects = raw
            .get("objects")
        ...
        let last = raw
            ...
            .and_then(Value::as_str)
            .map(str::to_string);
        let next = match last.as_deref() {
            Some("0x") | Some("") | None => None,
    ...
    async fn call(&self, method: &str, params: Value) -> Result<Value> {
        ...
        if let Some(err) = env.get("error") {
            return Err(anyhow!("ckb rpc error: {err}"));
        }
        env.get_mut("result")
            .map(Value::take)
            .ok_or_else(|| anyhow!("ckb rpc {method}: no result"))
    }
}
